...
const SectionNode = memo(({ data, selected }: NodeProps) => {
	...
	return (
		<div
			className={cn(
				"bg-white rounded-lg border-2 shadow-sm w-3xs overflow-hidden transition-colors",
...
export const LogicEditor: React.FC<LogicEditorProps> = ({
	...
	useEffect(() => {
		setNodes((nds) => {
			...
			const newSectionNodes = sections.map((section, index) => {
				const existingNode = currentSectionNodes.find(
					(n) => n.id === section.id,
				);
		...
		setEdges((eds) => {
			...
			const otherEdges = eds.filter(
				(e) =>
					!sections.some(
						(s) =>
							e.source === s.id && sections.some((s2) => e.target === s2.id),
					),
			);
	...
	const onConnect = useCallback(
		(params: Connection) => {
			...
			if (
				sourceNode?.type === "sectionNode" &&
				targetNode?.type === "logicalOperatorNode"
			) {
