...
export const ConditionNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const allFields = availableSections.flatMap(s => s.fields.map(f => ({ ...f, sectionTitle: s.title })));
  ...
  return (
    ...
    `}>
      ...
      <Handle type="target" position={Position.Top} className="!bg-slate-400 !w-3 !h-3 hover:!bg-orange-500 transition-colors" />
      ...
      <div className="bg-slate-50 p-2 border-t border-slate-100 flex justify-end items-center relative">
        ...
        <Handle type="source" position={Position.Right} id="else" className="!bg-slate-400 !w-3 !h-3" />
      </div>
    </div>
  );
});
